...
from supermercado.supermercado import super_utils as sutils
    ...
    unionedTiles = [
        {
            ...
            'properties': {},
            'type': 'Feature'
        } for feature, shapes in
        features.shapes(np.asarray(np.flipud(np.rot90(burn)).astype(np.uint8), order='C'), transform=aff) if shapes == 1
    ]
